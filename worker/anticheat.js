...
const TOTAL_LEVEL_WIDTH = 34116;
const MAX_SPEED = 1.8;
const TOTAL_GEMS = 936;
const TOTAL_LEVELS = 12;
const MIN_LEVEL_FRAMES = 744;
const MAX_CARRY_LIVES = 5;
const HIDDEN_LIVES = 4;
const BOSS_SCORE = 8000;
const MAX_PAR = 80;
const COMBO_SCORE = [200, 400, 800, 1000, 2000, 4000];
const LEVEL_MIN_FRAMES = [446, 845, 907, 838, 799, 920, 822, 836, 960, 810, 756, 875];
const LEVEL_MAX_SCORE = [24310, 72960, 75860, 83660, 13800, 120110, 91160, 94710, 76910, 110460, 18250, 97000];

/** Названия уровней - для сообщений бота. Тоже из игры, чтобы не разъехались. */
export const LEVEL_NAMES = ["Стажировка", "Галера", "Тестовое", "Аутсорс", "Облако", "HR-скрининг", "Стартап", "Легаси", "Техсобес", "Корпорация", "Хайлоад", "Оффер"];
...
export function checkRun(stats) {
   ...
   *
   * Жизни на финише уровня: с прошлого уровня переносится не больше
   * MAX_CARRY_LIVES, сверху - заработанное: сотня скиллов, цепочка после
   * шестого подряд и жизни, спрятанные в невидимых ящиках. Бонус за жизни
   * считается от этого достижимого максимума.
   */
  const maxLives =
    MAX_CARRY_LIVES + Math.floor(stats.skills / 100) + Math.floor(stats.stomps / 7) + HIDDEN_LIVES;
  ...
  const maxScore =
    ...
    stats.levelsCleared * (500 + maxLives * 250 + 3000 + MAX_PAR * 12);
...
/**
 * Грейд по пройденным мирам - та же шкала, что показывает игра: замок
 * каждого мира даёт его грейд, до первого замка - стажёр.
 */
